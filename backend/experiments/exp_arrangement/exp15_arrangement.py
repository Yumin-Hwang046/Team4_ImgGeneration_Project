...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "generated" / "merged" / "exp15_from_exp4" / "exp_arrangement"
...
class Job:
    ...
    contact_shadow_offset_y: int = 3
    cast_shadow_opacity: float = 0.12
    cast_shadow_blur: int = 24
    cast_shadow_offset_x: int = 6
    cast_shadow_offset_y: int = 10
    notes: str = ""
...
JOBS = [
    Job(
        ...
        contact_shadow_offset_y=1,
        cast_shadow_offset_x=0,
    ...
    Job(
        ...
        notes="front view, standing on dish",
    ),
    Job(
        job_name="toast",
        object_name="토스트",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Exp15 arrangement stage with fixed composition only.")
...
def append_log(
    *,
    ...
    with README_PATH.open("a", encoding="utf-8") as fp:
        ...
        fp.write(
            f"- 주요 파라미터: center=({layout_params['center_x']:.2f}, {layout_params['center_y']:.2f}), "
            f"scale={layout_params['scale']:.2f}, anchor=({layout_params['anchor_x']:.2f}, {layout_params['anchor_y']:.2f}), "
            f"bbox=({layout_params['bbox_w']:.2f}, {layout_params['bbox_h']:.2f}), "
            f"contact_shadow_opacity={job.contact_shadow_opacity}, contact_shadow_blur={job.contact_shadow_blur}, "
            f"cast_shadow_opacity={job.cast_shadow_opacity}, cast_shadow_blur={job.cast_shadow_blur}, seed={seed}\n"
        )
...
def build_initial_composite(job: Job, input_dir: Path) -> tuple[Image.Image, dict[str, float], Path, Path]:
    ...
    obj = transform_object(Image.open(input_path).convert("RGBA"), job)
    ...
    contact = alpha.filter(ImageFilter.GaussianBlur(radius=job.contact_shadow_blur))
    cast = alpha.filter(ImageFilter.GaussianBlur(radius=job.cast_shadow_blur))

    contact_layer = Image.new("RGBA", obj.size, (0, 0, 0, 0))
    contact_layer.putalpha(contact.point(lambda v: int(v * job.contact_shadow_opacity)))
    cast_layer = Image.new("RGBA", obj.size, (0, 0, 0, 0))
    cast_layer.putalpha(cast.point(lambda v: int(v * job.cast_shadow_opacity)))

    frame.alpha_composite(cast_layer, (x + job.cast_shadow_offset_x, min(FRAME_SIZE - oh, y + job.cast_shadow_offset_y)))
    frame.alpha_composite(contact_layer, (x, min(FRAME_SIZE - oh, y + job.contact_shadow_offset_y)))
    ...
    layout_params = {
        ...
        "bbox_w": ow / FRAME_SIZE,
        "bbox_h": oh / FRAME_SIZE,
    }
    print(
        f"[layout] {job.job_name} center=({layout_params['center_x']:.2f},{layout_params['center_y']:.2f}) "
        f"scale={layout_params['scale']:.2f} anchor=({job.anchor_x:.2f},{job.anchor_y:.2f}) "
        f"bbox=({layout_params['bbox_w']:.2f},{layout_params['bbox_h']:.2f})"
    )
    ...
    for idx, job in enumerate(select_jobs(args.jobs)):
        ...
        try:
            ...
            append_log(
                ...
                seed=seed,
                layout_params={
                    "center_x": job.anchor_x,
                    "center_y": job.anchor_y,
                    "scale": -1.0,
                    "anchor_x": job.anchor_x,
                    "anchor_y": job.anchor_y,
                    "bbox_w": -1.0,
